...
总结:`add()`方法首先确定好添加元素后的ArrayList含有的元素量,之后对比元素量和容量查看是否要扩容。若要进行扩容，则扩容后的新容量为旧容量的1.5倍，==同时还要处理1.5倍容量仍然不足和最大容量限制两个问题，若仍然不足则以所需容量为新的容量==。之后则使用`System.arraycopy()`方法移动数组元素,再将元素放入数组中.
